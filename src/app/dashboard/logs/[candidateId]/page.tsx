  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-xs space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-neutral-50">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className={`inline-flex items-center rounded-full px-3 py-1 text-2xs font-extrabold border ${
              candidateData.overallRisk === 'CRITICAL'
      ...
      <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-xs space-y-4">
        <h2 className="text-base font-bold text-neutral-800 border-b border-neutral-50 pb-2">Violation History Timeline</h2>
        ...
        {candidateData.events.length === 0 ? (
          ...
          <div className="relative border-l border-neutral-200 pl-6 ml-4 space-y-8 pt-2 pb-2">
            {candidateData.events.map((e) => (
              <div key={e.id} className="relative">
                {/* Timeline node marker */}
                <span className={`absolute -left-[31px] top-1 h-3.5 w-3.5 rounded-full border-4 border-white shadow-xs ${
                  e.severity === 'CRITICAL'
                    ? 'bg-red-500'
                    : e.severity === 'HIGH'
                    ? 'bg-amber-500'
                ...
                <div className="bg-neutral-50/40 rounded-xl p-4 border border-neutral-100 hover:border-neutral-200 transition-colors">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className={`inline-block rounded-md px-1.5 py-0.5 text-4xs font-extrabold text-white ${
                        e.severity === 'CRITICAL'
                          ? 'bg-red-600'
                          : e.severity === 'HIGH'
                          ? 'bg-amber-500'
