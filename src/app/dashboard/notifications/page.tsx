  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Notifications & Activity Feed</h1>
          <p className="mt-1 text-sm text-neutral-500 font-medium">
            Recent updates on candidate submissions, invitations, and system events.
          </p>
        </div>
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="self-start sm:self-auto rounded-lg border border-neutral-200 bg-white px-3.5 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-50 transition-colors shadow-xs"
          >
            Mark all as read
          </button>
        )}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-neutral-100 bg-white p-12 text-center shadow-xs">
          ...
          <p className="mt-1 text-xs text-neutral-400">
            {filter === 'UNREAD' ? 'No unread notifications at this time.' : 'Activity logs will appear here.'}
          </p>
        </div>
      ) : (
          ...
          {filteredNotifications.map((n) => {
            ...
            return (
              <div
                ...
                className={`rounded-2xl border p-5 transition-all flex items-start gap-4 cursor-pointer ${
                  !n.isRead
                    ? 'border-brand-green-border bg-brand-green-light/20 shadow-2xs'
                    : 'border-neutral-100 bg-white hover:border-neutral-200 shadow-xs'
                }`}
              >
