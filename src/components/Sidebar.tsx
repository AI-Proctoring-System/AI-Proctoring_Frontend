...
export default function Sidebar() {
  ...
  return (
    <div className="flex h-full w-64 flex-col border-r border-neutral-100 bg-white">
      ...
      <nav className="flex-1 space-y-1 px-4 py-6">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <span className={isActive ? 'text-brand-green' : 'text-neutral-400 group-hover:text-neutral-600'}>
                {item.icon}
              </span>
              {item.name}
            </Link>
      ...
      {/* User Footer info */}
      <div className="border-t border-neutral-50 p-4 flex flex-col gap-3">
        <div className="flex items-center gap-3 px-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-green text-white text-sm font-bold shadow-xs">
        ...
        <button
          onClick={logout}
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-neutral-100 py-2 text-xs font-semibold text-neutral-500 hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all cursor-pointer"
        >
